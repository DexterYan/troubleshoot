...
func (c *CollectClusterResources) Collect(progressChan chan<- interface{}) (CollectorResult, error) {
	...
	for i := 1; i <= 3; i++ {
		...
		go func() {
			for c := range collectorList {
				start[c.Name] = time.Now()
				c.Run()
				collectorName := strings.ReplaceAll(c.Name, "-", " ")
				fmt.Printf("\n%s Collector took %v\n", strings.Title(collectorName), time.Since(start[c.Name]))
			}
			defer wg.Done()
		}()
	}
	...
	// 		start[crCollector.Name] = time.Now()
	// 		crCollector.Run()
	// 		collectorName := strings.ReplaceAll(crCollector.Name, "-", " ")
	// 		fmt.Printf("\n%s Collector took %v\n", strings.Title(collectorName), time.Since(start[crCollector.Name]))
...
func readClusterCollectorList(clusterResourceCollectors []getClusterResource) <-chan getClusterResource {
	out := make(chan getClusterResource)
	go func() {
		for _, c := range clusterResourceCollectors {
			out <- c
		}
		defer close(out)
	}()
